...
export const _mock = {
  ...
  // Text
  houseTitle: (index: number) => _housesTitle[index],
  hostType: (index: number) => _housesHostType[index],
  ...
  number: {
    ...
    //house
    pricePerNight: (index: number) => _housesPricePerNight[index],
    totalPrice: (index: number) => _housesTotalPrice[index],
  },
  // Image
  image: {
    house: (length: number) => {
      const randomIndex = Math.floor(Math.random() * length) + 1;
      return `${CONFIG.assetsDir}/assets/images/mock/house/house-${randomIndex}.webp`;
    },
    cover: (index: number) =>
